...
#'   layout(1)
#'   barplot(w)
nrbm2 <- function(riskFun,LAMBDA=1,MAX_ITER=1000L,EPSILON_TOL=0.01,w0=0,maxCP=100L,convexRisk=TRUE,LowRankQP.method="LU") {
  ...
  A <- matrix(c(at,at),2L,byrow=TRUE);b <- c(bt,bt);s <- c(st,st)
